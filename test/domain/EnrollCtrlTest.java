...
public class EnrollCtrlTest {
	...
	private ArrayList<Offering> requestedOfferings(Course...courses) {
		...
		Term newTerm = new Term("newTerm");
		ArrayList<Offering> result = new ArrayList<>();
		for (Course course : courses) {
			cal.add(Calendar.DATE, 1);
			result.add(new Offering(course, newTerm, cal.getTime()));
		}
		return result;
	}
	...
	private boolean hasTaken(Student s, Course...courses) {
		...
		for (StudyRecord sr : s.getStudyRecords())
			if (sr.getOffering().getTerm().getName().equals("newTerm"))
				coursesTaken.add(sr.getOffering().getCourse());
		for (Course course : courses) {
	...
	public void cannotTakeWithoutPrePassed() throws EnrollmentRulesViolationException {
		bebe.takeOffering(new Offering(phys1, new Term("t1")), 18);
		bebe.takeOffering(new Offering(prog, new Term("t1")), 12);
		bebe.takeOffering(new Offering(math1, new Term("t1")), 8.4);
	...
	public void canTakeWithPreFinallyPassed() throws EnrollmentRulesViolationException {
		...
		bebe.takeOffering(new Offering(phys2, new Term("t2")), 10);
		bebe.takeOffering(new Offering(ap, new Term("t2")), 16);
		bebe.takeOffering(new Offering(math1, new Term("t2")), 10.5);
	...
	public void cannotTakeOfferingsWithSameExamTime() throws EnrollmentRulesViolationException {
		...
		new EnrollCtrl().enroll(bebe,
				List.of(
					new Offering(phys1, new Term("newTerm"), cal.getTime()),
					new Offering(math1, new Term("newTerm"), cal.getTime()),
					new Offering(phys1, new Term("newTerm"), cal.getTime())
				));
	}
	...
	public void canTake14WithGPA11() throws EnrollmentRulesViolationException {
		bebe.takeOffering(new Offering(phys1, new Term("t1")), 13);
		bebe.takeOffering(new Offering(prog, new Term("t1")), 11);
		bebe.takeOffering(new Offering(math1, new Term("t1")), 9);
	...
	public void canTake15WithGPA12() throws EnrollmentRulesViolationException {
		bebe.takeOffering(new Offering(phys1, new Term("t1")), 15);
		bebe.takeOffering(new Offering(prog, new Term("t1")), 12);
		bebe.takeOffering(new Offering(math1, new Term("t1")), 9);
	...
	public void canTake15WithGPA15() throws EnrollmentRulesViolationException {
		bebe.takeOffering(new Offering(phys1, new Term("t1")), 15);
		bebe.takeOffering(new Offering(prog, new Term("t1")), 15);
		bebe.takeOffering(new Offering(math1, new Term("t1")), 15);
	...
	public void canTake20WithGPA16() throws EnrollmentRulesViolationException {
		bebe.takeOffering(new Offering(phys1, new Term("t1")), 16);
		bebe.takeOffering(new Offering(prog, new Term("t1")), 16);
		bebe.takeOffering(new Offering(math1, new Term("t1")), 16);
